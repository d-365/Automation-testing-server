        ...
        if (driver!=null){
            ...
            if (caseIds.size()!=0){
                for (Integer caseId : caseIds) {
                    List<UIConsole> execCaseResult = new ExecAppCase(caseId,driver).call();
                    if (web.getWebCaseResult(execCaseResult,null)){
                        caseSuccess++;
                    }else {
                        caseFailed++;
                    }
                    ...
                    planResultDetail.setPlanResultId(planResult.getId());
                    planResultDetail.setResultConsole(execCaseResult);
                    planResultDetailMapper.insert(planResultDetail);
                }
            }
            // 4 执行计划后置动作
            ...
            mobilePhoneMapper.updateById(execPhone);
            // 5.2 更新用例执行结果
            planService.planEndResult(null,0,caseSuccess,caseFailed,planResult);
            // 5.3 关闭AppiumDriver
            driver.quit();
            ...
            if(isSendEmail == 1){
                RunPlanServiceImpl.planSendMail(plan,planResult.getId(),1, 1);
